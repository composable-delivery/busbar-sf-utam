...
mod tests {
    use super::*;
    use crate::driver::{ElementHandle, ShadowRootHandle};
    use async_trait::async_trait;

    // ── Minimal driver/element doubles for the phase-2 discovery loop ──
    //
    // `find_known_page_objects` is browser-only orchestration: a phase-1
    // `execute_script` bitmap, then a per-candidate `find_elements` +
    // `confirm_page_object_match`, each wrapped in a `tokio::time::timeout`.
    // These doubles let us drive that loop deterministically so the timeout
    // branches are exercised under unit tests rather than only on a live org.
    ...
    impl ElementHandle for StubElement {
        ...
        async fn find_elements(&self, _: &Selector) -> RuntimeResult<Vec<Box<dyn ElementHandle>>> {
            // No anchor children — confirm_page_object_match never calls this
            // for a PO with no anchors; return empty for safety.
            Ok(Vec::new())
        }
    }

    /// Driver double for the phase-2 loop. `bitmap` is returned verbatim from
    /// the phase-1 `execute_script`; `root_found` controls how many root
    /// elements `find_elements` yields per candidate.
    ...
    impl UtamDriver for DiscoveryStubDriver {
        ...
        async fn find_elements(&self, _: &Selector) -> RuntimeResult<Vec<Box<dyn ElementHandle>>> {
            Ok((0..self.root_found)
                .map(|_| Box::new(StubElement) as Box<dyn ElementHandle>)
                .collect())
        }
    ...
    fn single_po_registry() -> PageObjectRegistry {
        let registry = PageObjectRegistry::new();
        // Anchorless root PO: confirm_page_object_match short-circuits to
        // true (total_anchors == 0), so a found root yields a clean match.
        registry
    ...
    async fn find_known_page_objects_confirms_matched_candidate() {
        ...
        let registry = single_po_registry();
        let matched = find_known_page_objects(&driver, &registry).await.unwrap();
