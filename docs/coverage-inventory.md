...
## Page-object coverage report

CI now publishes a page-object-centered coverage artifact alongside the normal
Rust line coverage and Allure report. This keeps the review model explicit:

- **definition coverage**: each `.utam.json` parsed and code-generated by
  `utam-runtime/tests/compiled_page_objects.rs`;
- **live contract coverage**: each discovered root page object exercised in a
  real browser/org, with per-driver and per-page-context method/element counts;
- **behavioral coverage**: curated checks whose actions assert an observable
  outcome, not just selector resolution.

The merged outputs are written by `scripts/build_pageobject_coverage_report.py`:

```sh
PAGEOBJECT_COVERAGE_DIR="$PWD/target/pageobject-coverage" \
  cargo test -p utam-runtime --test compiled_page_objects --all-features -- --nocapture

python3 scripts/build_pageobject_coverage_report.py \
  --definition target/pageobject-coverage/definition-coverage.json \
  --allure-results /tmp/allure-results \
  --output-dir target/pageobject-coverage
```

The output directory contains:

- `pageobjects.json` — machine-readable merged status per page object;
- `pageobjects.csv` — flat review table for sorting/filtering;
- `summary.md` — GitHub step-summary text with inventory, definition, live,
  driver/context, and review-queue counts.

The unit CI job publishes a definition-only artifact. The Salesforce
Integration workflow merges that artifact with WebDriver + CDP Allure results
and publishes the merged JSON/CSV under the branch's `gh-pages` report.
